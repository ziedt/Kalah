...
public class Kallah extends JFrame implements MouseListener {
	...
	protected void decode(String s) {
		...
		if(tab[0].equals("human")){
			...
			if(playerId == 1){
				...
				mPanel.setButtonText("P1");
			} else {
				mShouldPlayer2Play = true;
				mPanel.setButtonText("P2");
			}
	...
	public static void main(String[] args) { 
		...
		String[] possibilities = {"java_player","random_player","greedy_player","most_seed_player","end_in_store_player","minimax_def_player",
				"alphabeta_simple_player","alphabeta_diff_player","alphabeta_potential_player","alphabeta_nbNonEmpty_player","alphabeta_totalSeeds_player",
				"alphabeta_ultimate_player"};
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(mShouldPlayer1Play && e.getY() > 209 && e.getY() < 275 && num != -1){
			...
			String cmd = num+"."; 
			writeProlog(cmd);
		} else if(mShouldConfirm && e.getY() > 434 && e.getY() < 528 &&
				e.getX() > 682 && e.getX() < 778){
	...
	private class ImagePanel extends JPanel{
		...
		public void setButtonText(String mtext){
			mButtonText = mtext;
			repaint();
		}
...
//				img = ImageIO.read(new File("img/fond.png"));
			...
			g.drawString(mButtonText, 695, 500);
